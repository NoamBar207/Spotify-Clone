...
export const SongDeatails = () => {
  ...
  if (!Object.keys(currSong).length)
    return <section className="song-deatails-container">Loadinggggg</section>;
  return (
    <section
      className="song-deatails-container"
      style={{
        background: `linear-gradient(rgba(0,0,0,.6) 0, #121212 80%), ${bgc}`,
      }}
    >
      <header className="song-deatails-header">
        <ThreeDots song={currSong} />
        <div className="song-deatails-station">
          Playing Song From {currStation.name} Playlist
        </div>
      ...
      <div className="song-deatails-img">
        <img alt="" src={currSong.snippet?.thumbnails.high.url} />
      </div>
    </section>
  );
};
