    ...
    return (
        <main className="login-signup-container">
            {Object.keys(currUser).length ?
                <section className='user-logo-container' onClick={() => utilService.toggleDisplayNone(modalRef)}>
                ...
                <>
                    {(window.innerWidth <= 480 && !Object.keys(currUser).length) &&
                        <div className="homepage-settings" onClick={() => utilService.toggleDisplayNone(modalRef)}><span><i class="fa-solid fa-gear"></i></span></div>}
                    <div className='search-result-container hide-none' ref={modalRef}>
                        <div className='search-result' onClick={() => { onLoginSignUp(false) }}><span><i className="fa-solid fa-right-from-bracket"></i></span>Log in</div>
                        <div className='search-result' onClick={() => { onLoginSignUp(true) }}><span><i className="fa-solid fa-envelope"></i></span>Sign Up</div>
                    </div>
                </>
            }
        </main>
