...
async function addSong(song) {
  ...
  const resUpload = await uploadService.uploadImg(
...
async function addSongToLike(currSong, user, isSongLiked) {
  ...
  // })]
  let userToReturn;
  if (currSong.etag) currSong = songService.getById(currSong.id.videoId);
  if (!Object.keys(user).length)
    return localStorageService.addSongToLikeStation(currSong);
  else {
    userToReturn = { ...user };
    if (isSongLiked) {
      ...
      const songToAdd = currSong.snippet.thumbnails.high.url.includes(
        "cloudinary"
      )
        ? currSong
        : await getById(currSong.videoId);
